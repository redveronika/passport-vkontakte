...
function verifyWrapper(options, verify) {
  ...
  return function passportVerify(req, accessToken, refreshToken, params, profile, verified) {
	...
	if(arity == 6) {
	  if(options.passReqToCallback) verify(req, accessToken, refreshToken, params, profile, verified);
	  else this.error(new Error('VKontakteStrategy: supplied 6 parameters, but passReqToCallback is false'));
	}
    else if (arity == 5) {
	  if(options.passReqToCallback) verify(req, accessToken, refreshToken, profile, verified);
	  else verify(accessToken, refreshToken, params, profile, verified);
    }
    else if (arity == 4) {
